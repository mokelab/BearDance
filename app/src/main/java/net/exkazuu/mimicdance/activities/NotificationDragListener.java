...
/**
 * Created by t-yokoi on 2015/11/06.
 */
public class NotificationDragListener implements OnTouchListener {
    ...
    public NotificationDragListener(Activity activity, ImageView[][] cellIcons, String[][] cellTexts) {
    ...
    public boolean onTouch(View view, MotionEvent event) {
        ...
        Location to = new Location(
            ...
            Math.max( (1200-left) / cellIcons[0][0].getWidth() - 1, 0));
    ...
    private void setNotificationCode() {
        StringBuilder builder = new StringBuilder();
        for (int row = 0; row < cellTexts.length; row++) {
            builder.append(Joiner.on(' ').join(cellTexts[row]).trim());
            builder.append('\n');
        }
        activity.getIntent().putExtra("NotificationCode", builder.toString().trim());
    }
    ...
    private static class Location {
        ...
        public boolean isWritableArea() {
            return 0 <= row && row <= 11 && 0 <= column && column <= 1;
        }

        public boolean isDustboxArea() {
//            return 11 <= row && row <= 12 && 2 <= column && column <= 4;
            return !isWritableArea();
        }
    }
